(() => {
  ...
  const ACCESSIBILITY_ANALYZER = {
    ...
    getTouchTargetSize(element) {
      ...
      const { minWidth, minHeight } = this.config.touchTargets;
    ...
    checkAriaLabels(element) {
      ...
      ['button', 'link', 'textbox', 'combobox', 'listbox', 'tab', 'menuitem'].includes(role);
...
// Анализ текста
    ...
    suggestContrastFix(data) {
      const { textColor, backgroundColor, requirements } = data;
    ...
    suggestAriaFix(element, type) {
      ...
      return suggestions[type] || { 'review': 'Требуется ручная проверка' };
    },
...
<div style="font-size: 20px; font-weight: bold; color: #ff4757;">${criticalCount}</div>
    ...
    getAccessibilityCategoryStats() {
      const stats = { visual: 0, keyboard: 0, semantic: 0, motor: 0, cognitive: 0, other: 0 };
    ...
    createFilterTabs() {
      ...
      const filters = [
        { name: 'all', label: 'Все', icon: '📋', color: '#667eea' },
        { name: 'critical', label: 'Критические', icon: '🚨', color: '#ff4757' },
        { name: 'visual', label: 'Визуальные', icon: '👁️', color: '#5f27cd' },
        { name: 'keyboard', label: 'Клавиатура', icon: '⌨️', color: '#00d2ff' },
        { name: 'semantic', label: 'Семантика', icon: '🏷️', color: '#10ac84' },
        { name: 'wcag-aa', label: 'WCAG AA', icon: '✓', color: '#ffa502' },
        { name: 'wcag-aaa', label: 'WCAG AAA', icon: '✓✓', color: '#ff6348' }
      ];
      ...
      filters.forEach((filter, index) => {
        ...
        tab.addEventListener('click', () => this.filterAccessibilityIssues(filter.name, tab));
    ...
    createCategoryBadge(issue) {
      const categories = {
        'visual': { icon: '👁️', color: '#5f27cd', label: 'Visual' },
        'keyboard': { icon: '⌨️', color: '#00d2ff', label: 'Keyboard' },
        'semantic': { icon: '🏷️', color: '#10ac84', label: 'Semantic' },
        'motor': { icon: '🤲', color: '#ff6348', label: 'Motor' },
        'cognitive': { icon: '🧠', color: '#764ba2', label: 'Cognitive' }
      };
      
      const cat = categories[issue.category] || { icon: '❓', color: '#666', label: 'Other' };
    ...
    createSeverityBadge(issue) {
      const severities = {
        'critical': { label: 'КРИТИЧНО', color: '#ff4757' },
        'warning': { label: 'ВНИМАНИЕ', color: '#ffa502' },
        'info': { label: 'ИНФО', color: '#3742fa' }
      };
    ...
    calculateTotalFixTime(issues) {
      const timeMap = {
        'contrast-aa': 10, 'contrast-aaa': 10, 'focus-accessibility': 15,
        'missing-accessible-name': 8, 'missing-required-state': 5,
        'touch-target-size': 12, 'readability-issues': 20
      };
    ...
    getAccessibilityPriority(issue) {
      ...
      const categoryWeight = { 
        semantic: 0.1, keyboard: 0.2, visual: 0.3, motor: 0.4, cognitive: 0.5 
      };
    ...
    getCategoryBreakdown(issues) {
      ...
      issues.forEach(issue => {
        if (!breakdown[issue.category]) {
          breakdown[issue.category] = { count: 0, severity: { critical: 0, warning: 0, info: 0 } };
        }
    ...
    generateQuickWins(issues) {
      const quickWins = issues.filter(issue => 
        issue.automationPossible && 
        ['missing-accessible-name', 'contrast-aa', 'touch-target-size'].includes(issue.issueType)
      );
    ...
    getAllWCAGCriteria() {
      return [
        { name: '1.4.3 Contrast (Minimum)', level: 'AA', description: 'Text contrast ratio of at least 4.5:1' },
        { name: '1.4.6 Contrast (Enhanced)', level: 'AAA', description: 'Text contrast ratio of at least 7:1' },
        { name: '2.4.7 Focus Visible', level: 'AA', description: 'Keyboard focus indicator is visible' },
        { name: '4.1.2 Name, Role, Value', level: 'A', description: 'Elements have accessible names and roles' },
        { name: '2.5.5 Target Size', level: 'AAA', description: 'Touch targets are at least 44x44px' },
        { name: '3.1.5 Reading Level', level: 'AAA', description: 'Content is readable at appropriate level' }
      ];
    },
