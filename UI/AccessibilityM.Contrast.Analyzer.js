(() => {
    ...
    const ACCESSIBILITY_ANALYZER = {
        ...
        analyzeAriaCompliance() {
            ...
            this.checkSemanticHTML5Elements();
            this.checkAriaLandmarks();
            this.checkSemanticRoles();
            this.checkSemanticAttributes();
            this.checkSemanticRelationships();
            this.checkMicrodata();
        },
        ...
        // === РАСШИРЕННЫЕ СЕМАНТИЧЕСКИЕ ПРОВЕРКИ ===

        // Проверка семантических HTML5 элементов
        checkSemanticHTML5Elements() {
            console.log('🏷️ Проверка семантических HTML5 элементов...');

            // Проверка наличия main элемента
            const mainElements = document.querySelectorAll('main');
            if (mainElements.length === 0) {
                this.addScreenReaderIssue('no-main-element', 
                    'Страница не содержит элемента main', []);
            } else if (mainElements.length > 1) {
                this.addScreenReaderIssue('multiple-main-elements', 
                    'Страница содержит несколько элементов main', Array.from(mainElements));
            }

            // Проверка структуры навигации
            ...
            navElements.forEach((nav, index) => {
                ...
                if (!hasLabel) {
                    this.addScreenReaderIssue('nav-no-label', 
                        `Навигация #${index + 1} не имеет подписи`, [nav]);
                }

                // Проверка списков в навигации
                const lists = nav.querySelectorAll('ul, ol');
                if (lists.length === 0) {
                    this.addScreenReaderIssue('nav-no-list', 
                        `Навигация #${index + 1} не содержит списка ссылок`, [nav]);
                }
            });

            // Проверка семантических секций
            this.checkSemanticSections();
            
            // Проверка заголовочных элементов
            this.checkHeaderElements();
            
            // Проверка футеров
            this.checkFooterElements();
        },

        // Проверка семантических секций
        checkSemanticSections() {
            const sections = document.querySelectorAll('section, article, aside');
            
            sections.forEach((section, index) => {
                const tagName = section.tagName.toLowerCase();
                
                // Проверка наличия заголовка в секции
                const hasHeading = section.querySelector('h1, h2, h3, h4, h5, h6');
                if (!hasHeading) {
                    this.addScreenReaderIssue('section-no-heading', 
                        `Элемент ${tagName} не содержит заголовка`, [section]);
                }

                // Проверка aria-label для секций без заголовков
                if (!hasHeading && !section.getAttribute('aria-label') && 
                    !section.getAttribute('aria-labelledby')) {
                    this.addScreenReaderIssue('section-no-label', 
                        `Элемент ${tagName} без заголовка должен иметь aria-label`, [section]);
                }

                // Специальные проверки для article
                if (tagName === 'article') {
                    const timeElements = section.querySelectorAll('time');
                    const hasTime = timeElements.length > 0;
                    
                    if (!hasTime) {
                        this.addScreenReaderIssue('article-no-time', 
                            'Статья не содержит информации о времени', [section]);
                    }
                }
            });
        },
        ...
        checkHeaderElements() {
            ...
            headers.forEach((header, index) => {
                ...
                const headerSiblings = siblings.filter(sibling => 
                    sibling.tagName.toLowerCase() === 'header' && sibling !== header);
                
                if (headerSiblings.length > 0) {
                    this.addScreenReaderIssue('multiple-headers', 
                        `Найдено несколько элементов header в одном контейнере`, [header, ...headerSiblings]);
                }

                // Проверка содержимого header
                const hasHeading = header.querySelector('h1, h2, h3, h4, h5, h6');
                const hasNav = header.querySelector('nav');
                const hasOtherContent = header.children.length > 0;
                
                if (!hasHeading && !hasNav && !hasOtherContent) {
                    this.addScreenReaderIssue('empty-header', 
                        'Элемент header пуст или не содержит семантического содержимого', [header]);
                }
            });
        },
        ...
        checkFooterElements() {
            ...
            footers.forEach((footer, index) => {
                ...
                const footerSiblings = siblings.filter(sibling => 
                    sibling.tagName.toLowerCase() === 'footer' && sibling !== footer);
                
                if (footerSiblings.length > 0) {
                    this.addScreenReaderIssue('multiple-footers', 
                        `Найдено несколько элементов footer в одном контейнере`, [footer, ...footerSiblings]);
                }

                // Проверка содержимого footer
                const hasContent = footer.children.length > 0 || footer.textContent.trim().length > 0;
                if (!hasContent) {
                    this.addScreenReaderIssue('empty-footer', 
                        'Элемент footer пуст', [footer]);
                }
            });
        },

        // Проверка ARIA landmarks
        checkAriaLandmarks() {
            console.log('🏷️ Проверка ARIA landmarks...');

            const landmarkRoles = ['banner', 'main', 'complementary', 'contentinfo', 'navigation', 'search'];
            const foundLandmarks = {};

            // Проверяем элементы с role
            document.querySelectorAll('[role]').forEach(element => {
                const role = element.getAttribute('role');
                if (landmarkRoles.includes(role)) {
                    if (foundLandmarks[role]) {
                        this.addScreenReaderIssue('duplicate-landmark', 
                            `Дублирование landmark роли: ${role}`, [foundLandmarks[role], element]);
                    } else {
                        foundLandmarks[role] = element;
                    }

                    // Проверка доступного имени для landmarks
                    const hasLabel = element.getAttribute('aria-label') || 
                                   element.getAttribute('aria-labelledby') ||
                                   element.querySelector('h1, h2, h3, h4, h5, h6');
                    
                    if (!hasLabel) {
                        this.addScreenReaderIssue('landmark-no-label', 
                            `Landmark ${role} не имеет подписи`, [element]);
                    }
                }
            });

            // Проверяем семантические HTML5 элементы как landmarks
            ...
            Object.entries(semanticLandmarks).forEach(([tag, role]) => {
                ...
                elements.forEach(element => {
                    if (foundLandmarks[role] && foundLandmarks[role] !== element) {
                        this.addScreenReaderIssue('duplicate-landmark', 
                            `Дублирование landmark: ${role} (${tag} и [role="${role}"])`, 
                            [foundLandmarks[role], element]);
                    } else if (!foundLandmarks[role]) {
                        foundLandmarks[role] = element;
                    }
                });
            });

            // Проверка обязательных landmarks
            if (!foundLandmarks.main) {
                this.addScreenReaderIssue('missing-main-landmark', 
                    'Отсутствует landmark main', []);
            }
        },

        // Проверка семантических ролей
        checkSemanticRoles() {
            console.log('🏷️ Проверка семантических ролей...');

            // Проверка правильного использования ролей
            this.checkButtonRoles();
            this.checkLinkRoles();
            this.checkFormRoles();
            this.checkListRoles();
            this.checkTableRoles();
        },

        // Проверка ролей кнопок
        checkButtonRoles() {
            const buttons = document.querySelectorAll('button, [role="button"]');
            
            buttons.forEach(button => {
                const hasAccessibleName = this.getAccessibleName(button);
                if (!hasAccessibleName) {
                    this.addScreenReaderIssue('button-no-name', 
                        'Кнопка не имеет доступного имени', [button]);
                }

                // Проверка на дублирование роли
                if (button.tagName === 'BUTTON' && button.getAttribute('role') === 'button') {
                    this.addScreenReaderIssue('redundant-role', 
                        'Избыточная роль button для элемента button', [button]);
                }

                // Проверка состояния кнопки
                const isPressed = button.getAttribute('aria-pressed');
                const isExpanded = button.getAttribute('aria-expanded');
                const isPressedButton = button.getAttribute('aria-pressed') !== null;
                const isToggleButton = button.getAttribute('aria-expanded') !== null;
                
                if (isPressedButton && isToggleButton) {
                    this.addScreenReaderIssue('conflicting-button-states', 
                        'Кнопка имеет конфликтующие состояния (pressed и expanded)', [button]);
                }
            });
        },

        // Проверка ролей ссылок
        checkLinkRoles() {
            const links = document.querySelectorAll('a, [role="link"]');
            
            links.forEach(link => {
                const hasHref = link.hasAttribute('href');
                const hasAccessibleName = this.getAccessibleName(link);
                
                if (!hasHref && !link.getAttribute('aria-label') && !link.getAttribute('aria-labelledby')) {
                    this.addScreenReaderIssue('link-no-href', 
                        'Ссылка без href должна иметь aria-label', [link]);
                }

                if (!hasAccessibleName) {
                    this.addScreenReaderIssue('link-no-name', 
                        'Ссылка не имеет доступного имени', [link]);
                }

                // Проверка на дублирование роли
                if (link.tagName === 'A' && link.getAttribute('role') === 'link') {
                    this.addScreenReaderIssue('redundant-role', 
                        'Избыточная роль link для элемента a', [link]);
                }
            });
        },
        ...
        checkFormRoles() {
            ...
            forms.forEach(form => {
                ...
                if (!hasLabel) {
                    this.addScreenReaderIssue('form-no-label', 
                        'Форма не имеет подписи', [form]);
                }

                // Проверка на дублирование роли
                if (form.tagName === 'FORM' && form.getAttribute('role') === 'form') {
                    this.addScreenReaderIssue('redundant-role', 
                        'Избыточная роль form для элемента form', [form]);
                }
            });
        },

        // Проверка ролей списков
        checkListRoles() {
            const lists = document.querySelectorAll('ul, ol, [role="list"]');
            
            lists.forEach(list => {
                const hasLabel = list.getAttribute('aria-label') || 
                               list.getAttribute('aria-labelledby');
                
                if (!hasLabel) {
                    this.addScreenReaderIssue('list-no-label', 
                        'Список не имеет подписи', [list]);
                }

                // Проверка на дублирование роли
                if ((list.tagName === 'UL' || list.tagName === 'OL') && list.getAttribute('role') === 'list') {
                    this.addScreenReaderIssue('redundant-role', 
                        'Избыточная роль list для элемента списка', [list]);
                }
            });
        },

        // Проверка ролей таблиц
        checkTableRoles() {
            ...
            tables.forEach(table => {
                ...
                if (!hasLabel) {
                    this.addScreenReaderIssue('table-no-label', 
                        'Таблица не имеет подписи', [table]);
                }

                // Проверка на дублирование роли
                if (table.tagName === 'TABLE' && table.getAttribute('role') === 'table') {
                    this.addScreenReaderIssue('redundant-role', 
                        'Избыточная роль table для элемента table', [table]);
                }
            });
        },

        // Проверка семантических атрибутов
        checkSemanticAttributes() {
            console.log('🏷️ Проверка семантических атрибутов...');

            // Проверка атрибута lang
            this.checkLanguageAttributes();
            
            // Проверка атрибута dir
            this.checkDirectionAttributes();
            
            // Проверка атрибута datetime
            this.checkDateTimeAttributes();
            
            // Проверка атрибута title
            this.checkTitleAttributes();
        },

        // Проверка языковых атрибутов
        checkLanguageAttributes() {
            const htmlElement = document.documentElement;
            const htmlLang = htmlElement.getAttribute('lang');
            
            if (!htmlLang) {
                this.addScreenReaderIssue('no-html-lang', 
                    'HTML элемент не имеет атрибута lang', [htmlElement]);
            } else if (!this.isValidLanguageCode(htmlLang)) {
                this.addScreenReaderIssue('invalid-lang-code', 
                    `Недопустимый код языка: ${htmlLang}`, [htmlElement]);
            }

            // Проверка изменения языка в тексте
            const elementsWithLang = document.querySelectorAll('[lang]');
            elementsWithLang.forEach(element => {
                const lang = element.getAttribute('lang');
                if (!this.isValidLanguageCode(lang)) {
                    this.addScreenReaderIssue('invalid-lang-code', 
                        `Недопустимый код языка: ${lang}`, [element]);
                }
            });
        },

        // Проверка атрибутов направления текста
        checkDirectionAttributes() {
            ...
            elementsWithDir.forEach(element => {
                ...
                if (!validDirections.includes(dir)) {
                    this.addScreenReaderIssue('invalid-dir-value', 
                        `Недопустимое значение dir: ${dir}`, [element]);
                }
            });
        },

        // Проверка атрибутов времени
        checkDateTimeAttributes() {
            ...
            timeElements.forEach(time => {
                ...
                if (!datetime) {
                    this.addScreenReaderIssue('time-no-datetime', 
                        'Элемент time не имеет атрибута datetime', [time]);
                } else if (!this.isValidDateTime(datetime)) {
                    this.addScreenReaderIssue('invalid-datetime', 
                        `Недопустимый формат datetime: ${datetime}`, [time]);
                }

                // Проверка текстового содержимого
                const textContent = time.textContent?.trim();
                if (!textContent) {
                    this.addScreenReaderIssue('time-no-text', 
                        'Элемент time не содержит текста', [time]);
                }
            });
        },

        // Проверка атрибутов title
        checkTitleAttributes() {
            ...
            elementsWithTitle.forEach(element => {
                ...
                if (!title || title.trim().length === 0) {
                    this.addScreenReaderIssue('empty-title', 
                        'Пустой атрибут title', [element]);
                } else if (title.length > 200) {
                    this.addScreenReaderIssue('title-too-long', 
                        'Атрибут title слишком длинный (>200 символов)', [element]);
                }
            });
        },
        ...
        checkSemanticRelationships() {
            console.log('🏷️ Проверка семантических связей...');

            // Проверка связей между заголовками и секциями
            this.checkHeadingSectionRelationships();
            
            // Проверка связей в формах
            this.checkFormRelationships();
            
            // Проверка связей в таблицах
            this.checkTableRelationships();
            
            // Проверка связей в списках
            this.checkListRelationships();
        },

        // Проверка связей заголовков и секций
        checkHeadingSectionRelationships() {
            const sections = document.querySelectorAll('section, article, aside');
            ...
            sections.forEach(section => {
                ...
                if (headings.length === 0) {
                    this.addScreenReaderIssue('section-no-heading', 
                        'Секция не содержит заголовков', [section]);
                } else if (headings.length > 1) {
                    // Проверка иерархии заголовков в секции
                    const headingLevels = Array.from(headings).map(h => parseInt(h.tagName.charAt(1)));
                    const firstHeading = headingLevels[0];
                    
                    for (let i = 1; i < headingLevels.length; i++) {
                        if (headingLevels[i] > firstHeading + 1) {
                            this.addScreenReaderIssue('heading-hierarchy-skip', 
                                `Пропуск в иерархии заголовков в секции`, [headings[i]]);
                        }
                    }
                }
            });
            ...
            forms.forEach(form => {
                ...
                inputs.forEach(input => {
                    ...
                    if (id) {
                        const label = document.querySelector(`label[for="${id}"]`);
                        if (!label) {
                            this.addScreenReaderIssue('input-no-associated-label', 
                                'Поле ввода не связано с label', [input]);
                        }
                    }
                });

                // Проверка группировки полей
                const fieldsets = form.querySelectorAll('fieldset');
                fieldsets.forEach(fieldset => {
                    const legend = fieldset.querySelector('legend');
                    if (!legend) {
                        this.addScreenReaderIssue('fieldset-no-legend', 
                            'Fieldset не содержит legend', [fieldset]);
                    }
                });
            });
        },
        ...
        checkTableRelationships() {
            ...
            tables.forEach(table => {
                ...
                // Проверка связей заголовков и ячеек
                cells.forEach(cell => {
                    const headersAttr = cell.getAttribute('headers');
                    if (headersAttr) {
                        const headerIds = headersAttr.split(/\s+/);
                        const validHeaders = headerIds.every(id => 
                            table.querySelector(`th[id="${id}"]`));
                        
                        if (!validHeaders) {
                            this.addScreenReaderIssue('invalid-headers-reference', 
                                'Недопустимые ссылки в атрибуте headers', [cell]);
                        }
                    }
                });
            });
        ...
        checkListRelationships() {
            ...
            lists.forEach(list => {
                ...
                if (items.length === 0) {
                    this.addScreenReaderIssue('empty-list', 
                        'Список не содержит элементов', [list]);
                }

                // Проверка вложенности списков
                ...
                nestedLists.forEach(nestedList => {
                    ...
                    if (parentItem && parentItem.tagName !== 'LI') {
                        this.addScreenReaderIssue('invalid-list-nesting', 
                            'Список вложен не в элемент li', [nestedList]);
                    }
                });
            });
        },
        ...
        checkMicrodata() {
            console.log('🏷️ Проверка микроразметки...');

            // Проверка JSON-LD
            this.checkJSONLD();
            
            // Проверка микроразметки Schema.org
            this.checkSchemaOrgMicrodata();
        ...
        checkJSONLD() {
            ...
            jsonLdScripts.forEach((script, index) => {
                try {
                    ...
                    if (!data['@context']) {
                        this.addScreenReaderIssue('jsonld-no-context', 
                            `JSON-LD #${index + 1} не содержит @context`, [script]);
                    }
                    
                    if (!data['@type']) {
                        this.addScreenReaderIssue('jsonld-no-type', 
                            `JSON-LD #${index + 1} не содержит @type`, [script]);
                    }
                } catch (error) {
                    this.addScreenReaderIssue('jsonld-invalid-json', 
                        `JSON-LD #${index + 1} содержит недопустимый JSON`, [script]);
                }
            });
        },

        // Проверка микроразметки Schema.org
        checkSchemaOrgMicrodata() {
            ...
            microdataElements.forEach(element => {
                ...
                if (!itemType) {
                    this.addScreenReaderIssue('microdata-no-type', 
                        'Элемент с itemscope не имеет itemtype', [element]);
                } else if (!itemType.startsWith('http://schema.org/') && 
                          !itemType.startsWith('https://schema.org/')) {
                    this.addScreenReaderIssue('microdata-invalid-type', 
                        `Недопустимый itemtype: ${itemType}`, [element]);
                }
            });
        },
        ...
        checkOpenGraph() {
            ...
            if (!ogTitle) {
                this.addScreenReaderIssue('missing-og-title', 
                    'Отсутствует Open Graph title', []);
            }
            
            if (!ogDescription) {
                this.addScreenReaderIssue('missing-og-description', 
                    'Отсутствует Open Graph description', []);
            }
            
            if (!ogImage) {
                this.addScreenReaderIssue('missing-og-image', 
                    'Отсутствует Open Graph image', []);
            }
        },

        // Вспомогательные методы для валидации
        isValidLanguageCode(lang) {
            // Простая проверка кода языка (ISO 639-1)
        ...
        isValidDateTime(datetime) {
            ...
            return dateTimePattern.test(datetime) || !isNaN(Date.parse(datetime));
        },
        ...
        addScreenReaderIssue(type, description, elements) {
            const criticalTypes = [
                'no-headings', 'input-no-label', 'table-no-headers',
                'no-main-element', 'missing-main-landmark', 'no-html-lang',
                'button-no-name', 'link-no-name', 'form-no-label',
                'table-no-label', 'input-no-associated-label', 'fieldset-no-legend'
            ];
            const severity = criticalTypes.includes(type) ? 'critical' : 'warning';
        ...
        getAccessibilityCategory(type) {
            const categories = {
                ...
                // Новые семантические проблемы
                'no-main-element': 'semantic',
                'multiple-main-elements': 'semantic',
                'nav-no-label': 'semantic',
                'nav-no-list': 'semantic',
                'section-no-heading': 'semantic',
                'section-no-label': 'semantic',
                'article-no-time': 'semantic',
                'multiple-headers': 'semantic',
                'empty-header': 'semantic',
                'multiple-footers': 'semantic',
                'empty-footer': 'semantic',
                'duplicate-landmark': 'semantic',
                'landmark-no-label': 'semantic',
                'missing-main-landmark': 'semantic',
                'button-no-name': 'semantic',
                'redundant-role': 'semantic',
                'conflicting-button-states': 'semantic',
                'link-no-href': 'semantic',
                'link-no-name': 'semantic',
                'form-no-label': 'semantic',
                'list-no-label': 'semantic',
                'table-no-label': 'semantic',
                'no-html-lang': 'semantic',
                'invalid-lang-code': 'semantic',
                'invalid-dir-value': 'semantic',
                'time-no-datetime': 'semantic',
                'invalid-datetime': 'semantic',
                'time-no-text': 'semantic',
                'empty-title': 'semantic',
                'title-too-long': 'semantic',
                'heading-hierarchy-skip': 'semantic',
                'input-no-associated-label': 'semantic',
                'fieldset-no-legend': 'semantic',
                'invalid-headers-reference': 'semantic',
                'empty-list': 'semantic',
                'invalid-list-nesting': 'semantic',
                'jsonld-no-context': 'semantic',
                'jsonld-no-type': 'semantic',
                'jsonld-invalid-json': 'semantic',
                'microdata-no-type': 'semantic',
                'microdata-invalid-type': 'semantic',
                'missing-og-title': 'semantic',
                'missing-og-description': 'semantic',
                'missing-og-image': 'semantic'
            };
        ...
        getWCAGCriteria(type) {
            const criteria = {
                ...
                // Новые семантические критерии
                'no-main-element': '1.3.1 Info and Relationships',
                'multiple-main-elements': '1.3.1 Info and Relationships',
                'nav-no-label': '2.4.6 Headings and Labels',
                'nav-no-list': '1.3.1 Info and Relationships',
                'section-no-heading': '1.3.1 Info and Relationships',
                'section-no-label': '2.4.6 Headings and Labels',
                'article-no-time': '1.3.1 Info and Relationships',
                'multiple-headers': '1.3.1 Info and Relationships',
                'empty-header': '1.3.1 Info and Relationships',
                'multiple-footers': '1.3.1 Info and Relationships',
                'empty-footer': '1.3.1 Info and Relationships',
                'duplicate-landmark': '1.3.1 Info and Relationships',
                'landmark-no-label': '2.4.6 Headings and Labels',
                'missing-main-landmark': '1.3.1 Info and Relationships',
                'button-no-name': '4.1.2 Name, Role, Value',
                'redundant-role': '4.1.2 Name, Role, Value',
                'conflicting-button-states': '4.1.2 Name, Role, Value',
                'link-no-href': '4.1.2 Name, Role, Value',
                'link-no-name': '4.1.2 Name, Role, Value',
                'form-no-label': '2.4.6 Headings and Labels',
                'list-no-label': '2.4.6 Headings and Labels',
                'table-no-label': '2.4.6 Headings and Labels',
                'no-html-lang': '3.1.1 Language of Page',
                'invalid-lang-code': '3.1.1 Language of Page',
                'invalid-dir-value': '1.3.1 Info and Relationships',
                'time-no-datetime': '1.3.1 Info and Relationships',
                'invalid-datetime': '1.3.1 Info and Relationships',
                'time-no-text': '1.3.1 Info and Relationships',
                'empty-title': '2.4.6 Headings and Labels',
                'title-too-long': '2.4.6 Headings and Labels',
                'heading-hierarchy-skip': '1.3.1 Info and Relationships',
                'input-no-associated-label': '1.3.1 Info and Relationships',
                'fieldset-no-legend': '1.3.1 Info and Relationships',
                'invalid-headers-reference': '1.3.1 Info and Relationships',
                'empty-list': '1.3.1 Info and Relationships',
                'invalid-list-nesting': '1.3.1 Info and Relationships',
                'jsonld-no-context': '1.3.1 Info and Relationships',
                'jsonld-no-type': '1.3.1 Info and Relationships',
                'jsonld-invalid-json': '1.3.1 Info and Relationships',
                'microdata-no-type': '1.3.1 Info and Relationships',
                'microdata-invalid-type': '1.3.1 Info and Relationships',
                'missing-og-title': '1.3.1 Info and Relationships',
                'missing-og-description': '1.3.1 Info and Relationships',
                'missing-og-image': '1.3.1 Info and Relationships'
            };
        ...
        canAutomate(type) {
            const automatable = [
                ...
                // Новые автоматизируемые семантические проблемы
                'no-main-element', 'multiple-main-elements', 'nav-no-label',
                'section-no-heading', 'section-no-label', 'multiple-headers',
                'empty-header', 'multiple-footers', 'empty-footer',
                'duplicate-landmark', 'landmark-no-label', 'missing-main-landmark',
                'button-no-name', 'redundant-role', 'link-no-name',
                'form-no-label', 'list-no-label', 'table-no-label',
                'no-html-lang', 'invalid-lang-code', 'invalid-dir-value',
                'time-no-datetime', 'invalid-datetime', 'time-no-text',
                'empty-title', 'title-too-long', 'empty-list',
                'invalid-list-nesting', 'jsonld-no-context', 'jsonld-no-type',
                'jsonld-invalid-json', 'microdata-no-type', 'microdata-invalid-type',
